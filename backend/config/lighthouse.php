...
return [
    ...
    */
    ...
    'route' => [
         ...
         */
        'middleware' => [
            // ROOT CAUSE found 2026-08-10: Lighthouse registers /graphql via $router->addRoute()
            // directly, NOT inside routes/web.php's closure — so it never got Laravel's 'web'
            // middleware group (EncryptCookies + StartSession) at all. The Google OAuth session
            // cookie was being set and sent correctly (verified with a controlled curl repro
            // using a manufactured session), but AttemptAuthentication below had no session to
            // read, so `me` always resolved to null no matter what CORS/withCredentials config
            // was correct. These two must run before AttemptAuthentication. VerifyCsrfToken is
            // deliberately NOT added — the explicit CORS origin allowlist in config/cors.php is
            // what protects the credentialed cross-origin POSTs instead (any content-type:
            // application/json POST triggers a preflight, which only whitelisted origins pass).
